...
class HomeDashboardApp:
            ...
            while True:
                if self.__vl53.sensor.data_ready():
                    ...
                    if valid:
                        near = min(valid)
                        # within threshold
                        if near < self.__detection_config.threshold_mm:
                            last_seen = time.monotonic()
                            if not self.screen_on:
                                reading = self.__lux_sensor.get_lux()  # read lux to update sensor state
                                self.logger.info(f"Object detected at {near} mm fade in @ lux={reading.lux:.2f}")                                                     
                            ...
                            if self.screen_on and (time.monotonic() - last_seen) > 2.0:
                                self.logger.info(f"Object at {near} mm (> {self.__detection_config.threshold_mm}) fade out")
                                self.__display_manager.fade_to(0)
                                self.screen_on = False

                if self.__environment_sensor_config.enabled:
                    await self.get_environment_readings()
    ...
    async def get_environment_readings(self):
        ...
        if readings:
            self.logger.info(f"Environment Readings: Temperature={readings.temperature_c:.2f}C, Humidity={readings.humidity_percent:.2f}%, Pressure={readings.pressure_hpa:.2f}hPa")
        else:
